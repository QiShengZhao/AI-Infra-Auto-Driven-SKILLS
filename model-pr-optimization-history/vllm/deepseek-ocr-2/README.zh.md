...
按 vllm 上游 `origin/main@ef54a4d604`、模型相关文件的 `git log --name-only -- <model-files>` 以及 GitHub Pull Request files API 生成。本页用于补齐 sgl-cookbook 中 `DeepSeek OCR 2` 缺失的历史 PR 优化文档。
...
- 动机: 标题「[Model] Upstream Deepseek-OCR model」；模型线: DeepSeek OCR 2；类别: 文档/测试/CI；主要 diff: `docs/models/supported_models.md`, `examples/offline_inference/vision_language.py`, `tests/models/registry.py`。
- 实现要点:
...
- 动机: 标题「[Bugfix] Fix deepseek-ocr multi-image inference and add `merge_by_field_config=True` with tensor schema support」；模型线: DeepSeek OCR 2；类别: 缺陷修复；主要 diff: `examples/offline_inference/vision_language_multi_image.py`, `tests/models/multimodal/processing/test_common.py`, `vllm/model_executor/models/deepseek_ocr.py`。
- 实现要点:
...
- 动机: 标题「[Bugfix] Validate custom logits processor xargs for online serving」；模型线: DeepSeek OCR 2；类别: 缺陷修复；主要 diff: `docs/design/logits_processors.md`, `docs/features/custom_arguments.md`, `docs/features/custom_logitsprocs.md`。
- 实现要点:
...
- 动机: 标题「[Model] Consolidate Deepseek-MoE implementation with DeepSeek-v2」；模型线: DeepSeek OCR 2；类别: 文档/测试/CI；主要 diff: `tests/models/registry.py`, `vllm/model_executor/models/deepseek.py`, `vllm/model_executor/models/deepseek_ocr.py`。
- 实现要点:
...
- 动机: 标题「Rename clashing method names for vLLM model protocol」；模型线: DeepSeek OCR 2；类别: 文档/测试/CI；主要 diff: `docs/contributing/model/basic.md`, `docs/contributing/model/multimodal.md`, `vllm/model_executor/models/apertus.py`。
- 实现要点:
...
- 动机: 标题「[BugFix] DeepSeek-OCR: apply NoRepeatNGramLogitsProcessor to greedy path」；模型线: DeepSeek OCR 2；类别: 缺陷修复；主要 diff: `vllm/model_executor/models/deepseek_ocr.py`。
- 实现要点:
...
- 动机: 标题「[Chore] Move tokenizer initialization methods」；模型线: DeepSeek OCR 2；类别: 模型支持/运行时入口；主要 diff: `benchmarks/benchmark_prefix_caching.py`, `benchmarks/benchmark_serving_structured_output.py`, `tests/compile/test_dynamic_shapes_compilation.py`。
- 实现要点:
...
- 动机: 标题「[Chore] Deprecate `merge_by_field_config` arg」；模型线: DeepSeek OCR 2；类别: 文档/测试/CI；主要 diff: `tests/models/multimodal/processing/test_common.py`, `tests/models/multimodal/processing/test_glm4_1v.py`, `tests/models/multimodal/processing/test_tensor_schema.py`。
- 实现要点:
...
- 动机: 标题「[Chore] Deprecate `SupportsMultiModal.merge_by_field_config`」；模型线: DeepSeek OCR 2；类别: 模型支持/运行时入口；主要 diff: `vllm/model_executor/models/aria.py`, `vllm/model_executor/models/aya_vision.py`, `vllm/model_executor/models/blip2.py`。
- 实现要点:
...
- 动机: 标题「[Renderer] Separate out `RendererConfig` from `ModelConfig`」；模型线: DeepSeek OCR 2；类别: 文档/测试/CI；主要 diff: `docs/contributing/model/transcription.md`, `tests/compile/distributed/test_sequence_parallelism.py`, `tests/compile/test_functionalization.py`。
- 实现要点:
...
- 动机: 标题「Revert "[Renderer] Separate out `RendererConfig` from `ModelConfig` (#30145)"」；模型线: DeepSeek OCR 2；类别: 缺陷修复；主要 diff: `docs/contributing/model/transcription.md`, `tests/compile/distributed/test_sequence_parallelism.py`, `tests/compile/test_functionalization.py`。
- 实现要点:
  - `docs/contributing/model/transcription.md` modified +6/-6
...
- 动机: 标题「feat: support LoRA for DeepSeek-OCR(Language Model part)」；模型线: DeepSeek OCR 2；类别: 模型支持/运行时入口；主要 diff: `docs/models/supported_models.md`, `vllm/model_executor/models/deepseek_ocr.py`。
- 实现要点:
...
- 动机: 标题「[Model] Standardize common vision encoders」；模型线: DeepSeek OCR 2；类别: 模型实现调整；主要 diff: `vllm/model_executor/models/clip.py`, `vllm/model_executor/models/deepencoder.py`, `vllm/model_executor/models/deepseek_ocr.py`。
- 实现要点:
...
- 动机: 标题「[Model] Remove redundant None check in DeepSeekOCR image input processing」；模型线: DeepSeek OCR 2；类别: 模型实现调整；主要 diff: `vllm/model_executor/models/deepseek_ocr.py`。
- 实现要点:
...
- 动机: 标题「[1/N] Reorganize multimodal processing code」；模型线: DeepSeek OCR 2；类别: 文档/测试/CI；主要 diff: `docs/api/README.md`, `docs/contributing/model/multimodal.md`, `docs/design/mm_processing.md`。
- 实现要点:
...
- 动机: 标题「[1/N] Initialize MM components in context managers (A-D)」；模型线: DeepSeek OCR 2；类别: 模型支持/运行时入口；主要 diff: `vllm/model_executor/models/aria.py`, `vllm/model_executor/models/audioflamingo3.py`, `vllm/model_executor/models/aya_vision.py`。
- 实现要点:
...
- 动机: 标题「[Models]: Make Multimodal config implicit in ViT implementation」；模型线: DeepSeek OCR 2；类别: 模型实现调整；主要 diff: `vllm/model_executor/layers/attention/mm_encoder_attention.py`, `vllm/model_executor/models/clip.py`, `vllm/model_executor/models/deepencoder.py`。
- 实现要点:
...
- 动机: 标题「[Chore] Update type annotation of `input_ids` in model forward」；模型线: DeepSeek OCR 2；类别: 文档/测试/CI；主要 diff: `docs/contributing/model/basic.md`, `tests/plugins/vllm_add_dummy_model/vllm_add_dummy_model/my_gemma_embedding.py`, `vllm/model_executor/models/afmoe.py`。
- 实现要点:
...
- 链接: https://github.com/vllm-project/vllm/pull/33165
- 状态/时间: merged / 2026-02-02
- 反查来源: `git log --name-only -- <model-files>` 或模型关键词补充；本卡按 GitHub Pull Request files API 审计。
- 代码 diff 已读范围: GitHub Pull Request files API 返回 9 个文件，+1099/-1，可读 patch 1159 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Model] Support DeepSeek-OCR-2」；模型线: DeepSeek OCR 2；类别: 模型支持/运行时入口；主要 diff: `docs/models/supported_models.md`, `examples/offline_inference/vision_language.py`, `tests/models/registry.py`。
- 实现要点:
  - `docs/models/supported_models.md` modified +1/-0
...
- 动机: 标题「[Models] Consolidate Deepseek-OCR2 processor」；模型线: DeepSeek OCR 2；类别: 模型实现调整；主要 diff: `vllm/model_executor/models/deepencoder2.py`, `vllm/model_executor/models/deepseek_ocr.py`, `vllm/model_executor/models/deepseek_ocr2.py`。
- 实现要点:
...
- 动机: 标题「[Multimodal] Expose `mm_processor_kwargs` for `DummyInputsBuilder`」；模型线: DeepSeek OCR 2；类别: 模型实现调整；主要 diff: `vllm/model_executor/models/aria.py`, `vllm/model_executor/models/audioflamingo3.py`, `vllm/model_executor/models/aya_vision.py`。
- 实现要点:
...
- 动机: 标题「Fix DeepSeek-OCR tensor validation for all size variants」；模型线: DeepSeek OCR 2；类别: 缺陷修复；主要 diff: `vllm/model_executor/models/deepseek_ocr.py`。
- 实现要点:
...
- 动机: 标题「[Refactor] Simplify dummy data generation」；模型线: DeepSeek OCR 2；类别: 文档/测试/CI；主要 diff: `docs/contributing/model/multimodal.md`, `tests/models/multimodal/processing/test_audioflamingo3.py`, `tests/models/multimodal/processing/test_common.py`。
- 实现要点:
...
- 动机: 标题「[Misc] Lazy import registered processors」；模型线: DeepSeek OCR 2；类别: 文档/测试/CI；主要 diff: `tests/models/registry.py`, `vllm/model_executor/models/deepseek_vl2.py`, `vllm/model_executor/models/fireredasr2.py`。
- 实现要点:
...
- 动机: 标题「[Bugfix][Model] Fix DeepSeek-OCR TensorSchema crash on empty images_crop」；模型线: DeepSeek OCR 2；类别: 缺陷修复；主要 diff: `tests/models/multimodal/processing/test_deepseek_ocr.py`, `vllm/model_executor/models/deepseek_ocr.py`。
- 实现要点:
...
- 动机: 标题「[Bugfix] Standardize custom HF Processor init」；模型线: DeepSeek OCR 2；类别: 缺陷修复；主要 diff: `vllm/model_executor/models/deepseek_ocr.py`, `vllm/model_executor/models/deepseek_ocr2.py`, `vllm/model_executor/models/glm4v.py`。
- 实现要点:
...
- 动机: 标题「[Misc] Reorganize inputs」；模型线: DeepSeek OCR 2；类别: 文档/测试/CI；主要 diff: `docs/api/README.md`, `docs/contributing/model/transcription.md`, `docs/features/multimodal_inputs.md`。
- 实现要点:
...
- 动机: 标题「[Examples] Resettle generate examples.」；模型线: DeepSeek OCR 2；类别: 文档/测试/CI；主要 diff: `.buildkite/test-amd.yaml`, `.buildkite/test_areas/misc.yaml`, `.buildkite/test_areas/model_runner_v2.yaml`。
- 实现要点:
...
- 动机: 标题「[MM][CG] Support ViT CG for Qwen2.5-VL」；模型线: DeepSeek OCR 2；类别: 模型支持/运行时入口；主要 diff: `docs/design/cuda_graphs_multimodal.md`, `examples/generate/multimodal/vision_language_offline.py`, `tests/models/multimodal/generation/test_qwen2_5_vl.py`。
- 实现要点:
...
- 动机: 标题「[MM][CG] Support ViT CG for Qwen2-VL」；模型线: DeepSeek OCR 2；类别: 模型支持/运行时入口；主要 diff: `docs/design/cuda_graphs_multimodal.md`, `examples/generate/multimodal/vision_language_offline.py`, `tests/models/multimodal/generation/test_vit_cudagraph.py`。
- 实现要点:
...
- 动机: 标题「[MM][Perf][CG] Support ViT full CUDA graph for Qwen3.5」；模型线: DeepSeek OCR 2；类别: 性能/后端优化；主要 diff: `docs/design/cuda_graphs_multimodal.md`, `examples/generate/multimodal/vision_language_offline.py`, `tests/models/multimodal/generation/test_vit_cudagraph.py`。
- 实现要点:
  - `docs/design/cuda_graphs_multimodal.md` modified +2/-1
...
- 动机: 标题「[MM][CG] Enable encoder Cudagraph for Step3VL」；模型线: DeepSeek OCR 2；类别: 模型支持/运行时入口；主要 diff: `docs/design/cuda_graphs_multimodal.md`, `examples/generate/multimodal/vision_language_offline.py`, `tests/models/multimodal/generation/test_vit_cudagraph.py`。
- 实现要点:
  - `docs/design/cuda_graphs_multimodal.md` modified +2/-0
  - `examples/generate/multimodal/vision_language_offline.py` modified +1/-0
