...
按 sglang 上游 `origin/main@5073c82a37`、模型相关文件的 `git log --name-only -- <model-files>` 以及 GitHub Pull Request files API 生成。本页用于补齐 sgl-cookbook 中 `Ring 2.5 1T` 缺失的历史 PR 优化文档。
...
- 动机: 标题「Support bailing moe」；模型线: Ring 2.5 1T；类别: 模型支持/运行时入口；主要 diff: `docs/supported_models/generative_models.md`, `python/sglang/srt/models/bailing_moe.py`, `test/srt/models/test_generation_models.py`。
- 实现要点:
...
- 动机: 标题「Support LingV2 model」；模型线: Ring 2.5 1T；类别: 模型支持/运行时入口；主要 diff: `benchmark/kernels/fused_moe_triton/tuning_fused_moe_triton.py`, `python/sglang/srt/configs/model_config.py`, `python/sglang/srt/layers/linear.py`。
- 实现要点:
...
- 动机: 标题「Fix Bailing MoE model bugs」；模型线: Ring 2.5 1T；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/bailing_moe.py`, `python/sglang/srt/server_args.py`。
- 实现要点:
...
- 动机: 标题「Refactor TopK to ensure readability and extensibility」；模型线: Ring 2.5 1T；类别: 模型实现调整；主要 diff: `python/sglang/srt/layers/moe/ep_moe/layer.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`, `python/sglang/srt/layers/moe/topk.py`。
- 实现要点:
...
- 动机: 标题「fix bailing_moe with enable_dp_attention」；模型线: Ring 2.5 1T；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/bailing_moe.py`。
- 实现要点:
...
- 动机: 标题「Fuse write kv buffer into rope for qwen3 moe & bailing moe」；模型线: Ring 2.5 1T；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/bailing_moe.py`, `python/sglang/srt/models/gpt_oss.py`, `python/sglang/srt/models/qwen3_moe.py`。
- 实现要点:
...
- 动机: 标题「Deprecate `global_server_args_dict`」；模型线: Ring 2.5 1T；类别: 模型实现调整；主要 diff: `python/sglang/global_config.py`, `python/sglang/srt/distributed/device_communicators/pynccl_allocator.py`, `python/sglang/srt/eplb/expert_location_dispatch.py`。
- 实现要点:
...
- 动机: 标题「bailingMoE: Fix Key error of deepep_mode」；模型线: Ring 2.5 1T；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/bailing_moe.py`。
- 实现要点:
  - `python/sglang/srt/models/bailing_moe.py` modified +2/-2
- 代码 diff 细节:
  - `python/sglang/srt/models/bailing_moe.py` modified +2/-2
...
- 动机: 标题「Revert "Deprecate `global_server_args_dict`"」；模型线: Ring 2.5 1T；类别: 缺陷修复；主要 diff: `python/sglang/global_config.py`, `python/sglang/srt/distributed/device_communicators/pynccl_allocator.py`, `python/sglang/srt/eplb/expert_location_dispatch.py`。
- 实现要点:
...
- 反查来源: `git log --name-only -- <model-files>` 或模型关键词补充；本卡按 GitHub Pull Request files API 审计。
- 代码 diff 已读范围: GitHub Pull Request files API 返回 54 个文件，+240/-321，可读 patch 1946 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Depreate `global_server_args_dict`」；模型线: Ring 2.5 1T；类别: 模型实现调整；主要 diff: `python/sglang/global_config.py`, `python/sglang/srt/distributed/device_communicators/pynccl_allocator.py`, `python/sglang/srt/eplb/expert_location_dispatch.py`。
- 实现要点:
  - `python/sglang/global_config.py` modified +0/-3
  - `python/sglang/srt/distributed/device_communicators/pynccl_allocator.py` modified +2/-2
  - `python/sglang/srt/eplb/expert_location_dispatch.py` modified +2/-2
...
- 反查来源: `git log --name-only -- <model-files>` 或模型关键词补充；本卡按 GitHub Pull Request files API 审计。
- 代码 diff 已读范围: GitHub Pull Request files API 返回 100 个文件，+90/-235，可读 patch 1164 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Lint] Add `python/sglang` to ruff F401 checks and remove unused imports in files」；模型线: Ring 2.5 1T；类别: 模型支持/运行时入口；主要 diff: `.pre-commit-config.yaml`, `python/sglang/srt/_custom_ops.py`, `python/sglang/srt/compilation/cuda_piecewise_backend.py`。
- 实现要点:
...
- 动机: 标题「[9/N] MoE Refactor: cleanup dispatcher interfaces」；模型线: Ring 2.5 1T；类别: 模型实现调整；主要 diff: `python/sglang/srt/layers/dp_attention.py`, `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`。
- 实现要点:
...
- 动机: 标题「Enable bailing_moe to support TP=16」；模型线: Ring 2.5 1T；类别: 模型支持/运行时入口；主要 diff: `python/sglang/srt/models/bailing_moe.py`。
- 实现要点:
...
- 动机: 标题「remove unecessary dual stream token threshold from the rest of models (qwen moe, kimi linear, etc.)」；模型线: Ring 2.5 1T；类别: 模型实现调整；主要 diff: `python/sglang/srt/models/bailing_moe.py`, `python/sglang/srt/models/kimi_linear.py`, `python/sglang/srt/models/llada2.py`。
- 实现要点:
...
- 动机: 标题「[bugfix] fix TBO crashes when attn_tp_size > 1」；模型线: Ring 2.5 1T；类别: 缺陷修复；主要 diff: `python/sglang/srt/batch_overlap/operations.py`, `python/sglang/srt/batch_overlap/two_batch_overlap.py`, `python/sglang/srt/layers/communicator.py`。
- 实现要点:
...
- 动机: 标题「Optimize Bailing-MoE with FlashInfer Fused All-Reduce」；模型线: Ring 2.5 1T；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/bailing_moe.py`。
- 实现要点:
...
- 动机: 标题「[Feature] JIT Fused QK norm + qk norm clean up」；模型线: Ring 2.5 1T；类别: 性能/后端优化；主要 diff: `python/sglang/jit_kernel/benchmark/bench_qknorm.py`, `python/sglang/jit_kernel/csrc/norm.cuh`, `python/sglang/jit_kernel/include/sgl_kernel/runtime.cuh`。
- 实现要点:
...
- 动机: 标题「Fix EPLB + FP4 Quantization Compatibility Issue」；模型线: Ring 2.5 1T；类别: 缺陷修复；主要 diff: `python/sglang/srt/layers/moe/utils.py`, `python/sglang/srt/models/bailing_moe.py`, `python/sglang/srt/models/deepseek_v2.py`。
- 实现要点:
...
- 动机: 标题「Use attn tp group in embedding for more models」；模型线: Ring 2.5 1T；类别: 模型实现调整；主要 diff: `python/sglang/srt/models/bailing_moe.py`, `python/sglang/srt/models/bailing_moe_nextn.py`, `python/sglang/srt/models/falcon_h1.py`。
- 实现要点:
...
- 动机: 标题「Fix the scenario where eh_proj is quantized in the bailing moe nextn weights」；模型线: Ring 2.5 1T；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/bailing_moe_nextn.py`。
- 实现要点:
...
- 动机: 标题「feat: Add Ling Flash v2.0 support for Eagle3」；模型线: Ring 2.5 1T；类别: 模型支持/运行时入口；主要 diff: `python/sglang/srt/models/bailing_moe.py`。
- 实现要点:
...
- 动机: 标题「Support LingV2_5 model」；模型线: Ring 2.5 1T；类别: 模型支持/运行时入口；主要 diff: `python/sglang/srt/configs/__init__.py`, `python/sglang/srt/configs/bailing_hybrid.py`, `python/sglang/srt/configs/model_config.py`。
- 实现要点:
...
- 动机: 标题「Cleanup debug log for Ring model」；模型线: Ring 2.5 1T；类别: 缺陷修复；主要 diff: `python/sglang/srt/configs/mamba_utils.py`, `python/sglang/srt/models/bailing_moe_linear.py`。
- 实现要点:
...
- 链接: https://github.com/sgl-project/sglang/pull/18829
- 状态/时间: merged / 2026-02-14
- 反查来源: `git log --name-only -- <model-files>` 或模型关键词补充；本卡按 GitHub Pull Request files API 审计。
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+66/-1，可读 patch 103 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Add ci test for ring model」；模型线: Ring 2.5 1T；类别: 模型支持/运行时入口；主要 diff: `python/sglang/test/accuracy_test_runner.py`, `test/registered/8-gpu-models/test_ring_2_5_1t.py`。
- 实现要点:
...
- 动机: 标题「update pre-commit config」；模型线: Ring 2.5 1T；类别: 文档/测试/CI；主要 diff: `.github/workflows/lint.yml`, `.pre-commit-config.yaml`, `3rdparty/amd/tuning/benchmark_moe_rocm.py`。
- 实现要点:
...
- 链接: https://github.com/sgl-project/sglang/pull/19201
- 状态/时间: merged / 2026-02-25
- 反查来源: `git log --name-only -- <model-files>` 或模型关键词补充；本卡按 GitHub Pull Request files API 审计。
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+739/-7，可读 patch 771 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Add server CUDA graph warmup CI step for cold H200 nodes」；模型线: Ring 2.5 1T；类别: 模型支持/运行时入口；主要 diff: `.github/workflows/pr-test.yml`, `scripts/ci/cuda/warmup_deep_gemm.py`, `scripts/ci/cuda/warmup_server.py`。
- 实现要点:
...
- 动机: 标题「Upgrade transformers==5.3.0」；模型线: Ring 2.5 1T；类别: 文档/测试/CI；主要 diff: `docs/advanced_features/vlm_query.ipynb`, `python/pyproject.toml`, `python/pyproject_cpu.toml`。
- 实现要点:
...
- 动机: 标题「[CPU] Add FP8 Bmm support」；模型线: Ring 2.5 1T；类别: 模型支持/运行时入口；主要 diff: `python/sglang/srt/models/bailing_moe_linear.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla_fused_rope_cpu.py`。
- 实现要点:
...
- 动机: 标题「fix fused_set_kv_buffer for rope with Ling-v2」；模型线: Ring 2.5 1T；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/bailing_moe.py`。
- 实现要点:
...
- 链接: https://github.com/sgl-project/sglang/pull/21751
- 状态/时间: merged / 2026-03-31
- 反查来源: `git log --name-only -- <model-files>` 或模型关键词补充；本卡按 GitHub Pull Request files API 审计。
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+2/-0，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[CI] Fix ring test timeout」；模型线: Ring 2.5 1T；类别: 缺陷修复；主要 diff: `test/registered/8-gpu-models/test_ring_2_5_1t.py`。
- 实现要点:
...
- 链接: https://github.com/sgl-project/sglang/pull/22045
- 状态/时间: merged / 2026-04-03
- 反查来源: `git log --name-only -- <model-files>` 或模型关键词补充；本卡按 GitHub Pull Request files API 审计。
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+5/-2，可读 patch 41 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[CI] Adjust CI server launch timeout」；模型线: Ring 2.5 1T；类别: 文档/测试/CI；主要 diff: `python/sglang/test/accuracy_test_runner.py`, `python/sglang/test/test_utils.py`, `test/registered/8-gpu-models/test_ring_2_5_1t.py`。
- 实现要点:
...
- 链接: https://github.com/sgl-project/sglang/pull/22267
- 状态/时间: merged / 2026-04-07
- 反查来源: `git log --name-only -- <model-files>` 或模型关键词补充；本卡按 GitHub Pull Request files API 审计。
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+3/-2，可读 patch 19 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Move ring test to nightly」；模型线: Ring 2.5 1T；类别: 文档/测试/CI；主要 diff: `test/registered/8-gpu-models/test_ring_2_5_1t.py`。
- 实现要点:
...
- 动机: 标题「[CI] Update est_time for 64 tests based on actual elapsed times」；模型线: Ring 2.5 1T；类别: 文档/测试/CI；主要 diff: `test/registered/4-gpu-models/test_nvidia_nemotron_3_super_nvfp4.py`, `test/registered/4-gpu-models/test_qwen35_hicache.py`, `test/registered/4-gpu-models/test_qwen3_30b.py`。
- 实现要点:
...
- 动机: 标题「Add new Mintlify documentation site (docs_new/)」；模型线: Ring 2.5 1T；类别: 模型支持/运行时入口；主要 diff: `.gitignore`, `docs_new/.github/workflows/sync-lmsys-sglang-blogs.yml`, `docs_new/.gitignore`。
- 实现要点:
...
- 动机: 标题「[Docs] Sync docs_new with legacy docs and update migration redirects」；模型线: Ring 2.5 1T；类别: 文档/测试/CI；主要 diff: `.pre-commit-config.yaml`, `docs_new/cookbook/autoregressive/DeepSeek/DeepSeek-Math-V2.mdx`, `docs_new/cookbook/autoregressive/DeepSeek/DeepSeek-OCR-2.mdx`。
- 实现要点:
...
- 动机: 标题「Apply should_use_dp_reduce_scatterv guard to remaining MoE models (follow-up to #23731)」；模型线: Ring 2.5 1T；类别: 模型实现调整；主要 diff: `python/sglang/srt/models/bailing_moe.py`, `python/sglang/srt/models/bailing_moe_linear.py`, `python/sglang/srt/models/deepseek_v2.py`。
- 实现要点:
...
- 动机: 标题「refactor(moe): centralize post-experts all-reduce skip predicate」；模型线: Ring 2.5 1T；类别: 模型实现调整；主要 diff: `python/sglang/srt/layers/moe/__init__.py`, `python/sglang/srt/layers/moe/utils.py`, `python/sglang/srt/models/bailing_moe.py`。
- 实现要点:
...
- 动机: 标题「[4/N] Quantization Refactor: AWQ schemes and Kernel call and weight init split」；模型线: Ring 2.5 1T；类别: 性能/后端优化；主要 diff: `python/sglang/srt/hardware_backend/gpu/quantization/awq_kernels.py`, `python/sglang/srt/hardware_backend/npu/quantization/awq_kernels.py`, `python/sglang/srt/layers/linear.py`。
- 实现要点:
...
- 动机: 标题「[SKILL] Upgrade sglang profile and auto_benchmark skills」；模型线: Ring 2.5 1T；类别: 文档/测试/CI；主要 diff: `agent-skills/llm-serving-auto-benchmark/SKILL.md`, `skills/llm-serving-auto-benchmark/configs/cookbook-llm/README.md`, `skills/llm-serving-auto-benchmark/configs/cookbook-llm/deepseek-math-v2.yaml`。
- 实现要点:
...
- 动机: 标题「nextn subclass owns post_load_weights is_nextn」；模型线: Ring 2.5 1T；类别: 模型实现调整；主要 diff: `python/sglang/srt/model_loader/loader.py`, `python/sglang/srt/model_loader/utils.py`, `python/sglang/srt/models/bailing_moe_nextn.py`。
- 实现要点:
