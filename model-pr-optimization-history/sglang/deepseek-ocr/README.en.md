...
| Merged | PR | Title | Tracked files touched |
| --- | --- | --- | --- |
| 2026-06-01 | [#25813](https://github.com/sgl-project/sglang/pull/25813) | docs(cookbook): port popular model usage guides into cookbook pages | `DeepSeek-OCR.mdx`, `deepseek_ocr.mdx` |
...
- Motivation: Title: "model: support deepseek-ocr"; model line: DeepSeek OCR; category: model support/runtime entry; main diff: `python/sglang/srt/configs/deepseek_ocr.py`, `python/sglang/srt/configs/deepseekvl2.py`, `python/sglang/srt/configs/model_config.py`.
- Key implementation:
...
- Motivation: Title: "[Bugfix]: distinguish processors for deepseek_vl2 and deepseek_ocr to p…"; model line: DeepSeek OCR; category: bug fix; main diff: `python/sglang/srt/configs/deepseek_ocr.py`, `python/sglang/srt/configs/deepseekvl2.py`, `python/sglang/srt/multimodal/customized_mm_processor_utils.py`.
- Key implementation:
...
- Motivation: Title: "Feat: deepseek-ocr logits processor"; model line: DeepSeek OCR; category: model implementation change; main diff: `python/sglang/srt/configs/deepseek_ocr.py`, `python/sglang/srt/sampling/custom_logit_processor.py`.
- Key implementation:
...
- Motivation: Title: "Fix lint in deepseek-ocr"; model line: DeepSeek OCR; category: bug fix; main diff: `python/sglang/srt/configs/deepseek_ocr.py`.
- Key implementation:
...
- Motivation: Title: "[NPU] supports ds-ocr model on ascend"; model line: DeepSeek OCR; category: model support/runtime entry; main diff: `python/sglang/srt/models/deepseek.py`, `python/sglang/srt/models/deepseek_ocr.py`.
- Key implementation:
...
- Motivation: Title: "Support DeepSeek-OCR-2 in SGLang (OCR2 vision pipeline, tokenization alignment, and weight loading fixes)#17833"; model line: DeepSeek OCR; category: bug fix; main diff: `docs/basic_usage/deepseek_ocr.md`, `docs/index.rst`, `docs/supported_models/multimodal_language_models.md`.
- Key implementation:
...
- Motivation: Title: "[XPU] Integrate MoE and minor improvements in XPU attention backend"; model line: DeepSeek OCR; category: docs/tests/CI; main diff: `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`, `python/sglang/srt/layers/moe/fused_moe_triton/moe_align_block_size.py`, `python/sglang/srt/layers/moe/moe_runner/triton.py`.
- Key implementation:
...
- Motivation: Title: "update pre-commit config"; model line: DeepSeek OCR; category: docs/tests/CI; main diff: `.github/workflows/lint.yml`, `.pre-commit-config.yaml`, `3rdparty/amd/tuning/benchmark_moe_rocm.py`.
- Key implementation:
...
- Motivation: Title: "Adapt the Qwen2Model._update_causal_mask for transformers==4.57.1"; model line: DeepSeek OCR; category: model implementation change; main diff: `python/sglang/srt/models/deepseek_ocr.py`.
- Key implementation:
...
- Motivation: Title: "fix: align DeepSeek OCR vision dtypes"; model line: DeepSeek OCR; category: bug fix; main diff: `python/sglang/srt/models/deepseek_ocr.py`.
- Key implementation:
...
- Motivation: Title: "[AMD] [DeepSeek-OCR-2 Day 0] Enable DeepSeek-OCR-2 on AMD GPUs and add nightly test"; model line: DeepSeek OCR; category: model support/runtime entry; main diff: `python/sglang/srt/models/deepseek_ocr.py`, `python/sglang/srt/multimodal/processors/base_processor.py`, `test/registered/amd/accuracy/mi30x/test_vlms_mmmu_eval_amd.py`.
- Key implementation:
...
- Motivation: Title: "Add Mistral Small 4 (Pixtral) support"; model line: DeepSeek OCR; category: model support/runtime entry; main diff: `benchmark/mmmu/bench_sglang.py`, `benchmark/mmmu/eval_utils.py`, `python/sglang/srt/configs/deepseek_ocr.py`.
- Key implementation:
...
- Motivation: Title: "[CPU] Fix MoE layer support for DeepSeek-OCR models"; model line: DeepSeek OCR; category: bug fix; main diff: `python/sglang/srt/models/deepseek.py`, `python/sglang/srt/models/deepseek_ocr.py`.
- Key implementation:
...
- Motivation: Title: "refactor: replace mm_inputs dict with MultimodalProcessorOutput"; model line: DeepSeek OCR; category: model implementation change; main diff: `python/sglang/srt/disaggregation/encode_receiver.py`, `python/sglang/srt/disaggregation/encode_server.py`, `python/sglang/srt/managers/io_struct.py`.
- Key implementation:
...
- Motivation: Title: "fix ut test_moe"; model line: DeepSeek OCR; category: bug fix; main diff: `test/srt/run_suite.py`, `test/srt/xpu/test_deepseek_ocr.py`, `test/srt/xpu/test_deepseek_ocr_triton.py`.
- Key implementation:
...
- Motivation: Title: "Add new Mintlify documentation site (docs_new/)"; model line: DeepSeek OCR; category: model support/runtime entry; main diff: `.gitignore`, `docs_new/.github/workflows/sync-lmsys-sglang-blogs.yml`, `docs_new/.gitignore`.
- Key implementation:
...
- Motivation: Title: "[XPU] Fix DeepSeek-OCR tests under transformers 5.x"; model line: DeepSeek OCR; category: bug fix; main diff: `test/srt/xpu/test_deepseek_ocr.py`, `test/srt/xpu/test_deepseek_ocr_triton.py`.
- Key implementation:
...
- Motivation: Title: "[Docs] Sync docs_new with legacy docs and update migration redirects"; model line: DeepSeek OCR; category: docs/tests/CI; main diff: `.pre-commit-config.yaml`, `docs_new/cookbook/autoregressive/DeepSeek/DeepSeek-Math-V2.mdx`, `docs_new/cookbook/autoregressive/DeepSeek/DeepSeek-OCR-2.mdx`.
- Key implementation:
...
- Motivation: Title: "Update XPU Docker runtime stack & hf_home config"; model line: DeepSeek OCR; category: docs/tests/CI; main diff: `.github/workflows/pr-test-xpu.yml`, `docker/xpu.Dockerfile`, `test/srt/xpu/test_deepseek_ocr.py`.
- Key implementation:
...
- Motivation: Title: "[FIX][1/2] fix step3-vl/deepseek-ocr image processor error"; model line: DeepSeek OCR; category: bug fix; main diff: `python/sglang/srt/multimodal/processors/step3_vl.py`.
- Key implementation:
...
- Motivation: Title: "chore: add vLLM SPDX copyright headers to ported files"; model line: DeepSeek OCR; category: model support/runtime entry; main diff: `benchmark/hicache/bench_serving.py`, `benchmark/kernels/fused_moe_triton/tuning_fused_moe_triton.py`, `benchmark/kernels/fused_moe_triton/tuning_fused_moe_triton_sep.py`.
- Key implementation:
...
- Motivation: Title: "[NPU] Support model DeepSeek-OCR and DeepSeek-OCR-2"; model line: DeepSeek OCR; category: model support/runtime entry; main diff: `python/sglang/srt/models/deepseek.py`.
- Key implementation:
...
- Motivation: Title: "Add Accuracy Benchmark for OCR models"; model line: DeepSeek OCR; category: model support/runtime entry; main diff: `.codespellrc`, `benchmark/ocr/README.md`, `benchmark/ocr/bench_sglang.py`.
- Key implementation:
...
- Motivation: Title: "[FIX][2/2] fix step3-vl/deepseek-ocr image processor error"; model line: DeepSeek OCR; category: bug fix; main diff: `python/sglang/srt/configs/deepseek_ocr.py`.
- Key implementation:
...
- Motivation: Title: "Use hf_transformers_utils.get_processor to load model"; model line: DeepSeek OCR; category: docs/tests/CI; main diff: `python/sglang/benchmark/utils.py`.
- Key implementation:
